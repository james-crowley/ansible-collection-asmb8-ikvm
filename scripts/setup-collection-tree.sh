...
MARKER="${BUILD_ROOT}/.asmb8-collection-build-root"
OWNER_PID_FILE="${BUILD_ROOT}/.asmb8-collection-build-root.pid"
...
if [ -e "${BUILD_ROOT}" ]; then
    ...
    if [ -f "${OWNER_PID_FILE}" ]; then
        ...
        if [ -n "${OWNER_PID}" ] && [ "${OWNER_PID}" != "$$" ] && kill -0 "${OWNER_PID}" 2>/dev/null; then
            ...
            echo "  export COLLECTION_BUILD_ROOT=\${TMPDIR:-/tmp}/asmb8-build-\$\$" >&2
            exit 1
        fi
    fi
