...
already cost real time on its sibling, `james_crowley.intel_amt`, and that will
apply here too because they are properties of `ansible-core` and this project's
own tooling, not of Intel AMT specifically.

This collection is pre-1.0 and, as of this writing, has no module code, no
`module_utils`, and no CI configuration yet. Most of what follows describes the
mechanics you will need once that content exists; it is written now so the
conventions are settled before the first module lands.

## Before you start: stage the collection tree

`ansible-test` insists the collection live at
`<root>/ansible_collections/james_crowley/asmb8_ikvm/`, regardless of where you
...
1. **It copies, not symlinks.** `ansible-test` resolves paths in ways that make a
   ...
   a snapshot that no longer matches your working tree.
2. **Set `COLLECTION_BUILD_ROOT` when more than one checkout might stage
   ...
   checkouts at the *same* path can collide on the same build root and clobber each
   other mid-run:
...
`requirements-dev.txt` pins exact versions on purpose — see its own comment for
why. Bump the pins in their own PR and fix whatever the new version finds there.

`ansible-core~=2.17.0` above is the collection's declared floor (`meta/runtime.yml`);
anything `>= 2.17` works locally.

## Local verification sequence

There is currently no CircleCI configuration in this repository — CI setup is
tracked as a separate piece of work. Until it exists, run this sequence locally
before opening a PR:
...
ansible-test integration --venv --python 3.12   # once mock servers exist
...
run `pytest` yourself, but **not from the repository root** — a test that imports
its subject as `ansible_collections.james_crowley.asmb8_ikvm.plugins.…` only
resolves if the collection is sitting inside an `ansible_collections/` directory
that is itself on `sys.path`. From the root you get:
...
files in the repository while running files under `${TMPDIR:-/tmp}` — a "my fix
had no effect" moment here is almost always a stale tree.

## Practical traps

These are not theoretical for this collection's tooling — each one has already
produced a real, confusing failure on `james_crowley.intel_amt`, which shares
this repository's `ansible-core` floor, doc-string conventions, and lint config.
...
`yamllint` sanity test fails. This has no example in this repository yet because
`plugins/doc_fragments/` is empty — watch for it the moment a connection doc
fragment is written.

### `ansible-core >= 2.21` needs `_ANSIBLE_PROFILE = "legacy"` in unit tests

Unit tests that drive `AnsibleModule` directly by setting
`basic._ANSIBLE_ARGS` (a `_set_module_args()` helper, typically duplicated across
...
`[tool.ruff.lint.per-file-ignores]` before reaching for an inline comment — in
particular `E402` in `plugins/modules/*.py`, ignored project-wide because Ansible's
module convention requires the `DOCUMENTATION`/`EXAMPLES`/`RETURN` string literals at
the top of the file, before any import.
...
filename is not load-bearing, only the contents are. `changelogs/config.yaml` sets
`keep_fragments: false`, so `antsibull-changelog release` **deletes** every fragment
once it has folded it into `CHANGELOG.md`/`CHANGELOG.rst`. The shape:
...
user-visible effect) do not need a fragment, but if in doubt, add one.

## Scope reminders

- `tests/hardware/inventory.yml` (once it exists) must be gitignored and must never
  contain real hostnames, credentials, or certificate fingerprints in a commit —
  commit only an `.example` file.
- Never commit real boot media (`*.iso`, `*.img` are gitignored except under test
  fixture directories).
- If you touch `docs/protocol-notes.md` (once it exists), treat the byte layouts and
  field mappings there as normative — "improving" them without a firmware capture or
  reference source to back the change is exactly the kind of unverified drift this
  project cannot afford, given that the iUSB protocol has no public specification to
  fall back on.
