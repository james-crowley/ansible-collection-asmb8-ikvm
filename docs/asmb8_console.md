...
**This module used to be named `asmb8_redirection`.** Before the first Galaxy
release, that name was found to be misleading: it is named after the sibling
[`james_crowley.intel_amt`](https://github.com/james-crowley/ansible-collection-intel-amt)
collection's `amt_redirection` module, which only *reports and toggles a
service-enablement flag* — it never opens a session at all. This module does
the opposite: it always opens a live session. The implementation below moved
here, essentially unchanged, when that confusion was found and fixed; see
[`docs/asmb8_redirection.md`](asmb8_redirection.md) for what took over the old
name.
...
`plugins/modules/asmb8_console.py`.
...
| `operation.action` | `str` | always | Always `"asmb8_console.capture"`. |
...
Verified against the `RETURN` block in `plugins/modules/asmb8_console.py`.
...
- Send keyboard or mouse input (`OP_HID_PKT` is recognised by
  ...
  by this module).
- Configure or persist any redirection-service setting on the BMC — this
  module only opens and closes a session; it has no "inspect current
  redirection configuration" or "enable/disable the service" behaviour. See
  [`asmb8_redirection`](asmb8_redirection.md) for that, now that the two are
  separate modules.

## See also

- [`asmb8_redirection`](asmb8_redirection.md) — reports (and, once a real RPC
  is confirmed, toggles) whether the BMC's own services are enabled, without
  ever opening a session. Used to share this module's name and be confused
  with it; read that module's own "Why this module exists" section for the
  full story.
- [`asmb8_media`](asmb8_media.md) — the other module that authenticates
  against `.asp` and fetches `jviewer.jnlp`, for virtual media rather than a
  console session.
