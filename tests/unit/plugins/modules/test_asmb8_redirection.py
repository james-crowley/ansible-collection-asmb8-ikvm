...
def _wire_no_network(monkeypatch, *, transport: _FakeTransport | None = None, token: str = TOKEN, kvm_secure: bool = False, client_ip: str = "10.1.1.1"):
    ...
    monkeypatch.setattr(asmb8_redirection, "resolve_token_and_security", lambda params: (token, kvm_secure, client_ip))
    monkeypatch.setattr(asmb8_redirection, "build_kvm_transport", lambda params, *, kvm_secure: transport)
    return transport
...
class TestArgumentSpec:
    ...
    def test_token_is_no_log(self):
        assert asmb8_redirection.argument_spec()["token"]["no_log"] is True

    def test_capture_choices_and_default(self):
        spec = asmb8_redirection.argument_spec()["capture"]
    ...
    def test_kvm_port_defaults_to_7578(self):
        assert asmb8_redirection.argument_spec()["kvm_port"]["default"] == 7578
    ...
    def test_kvm_secure_has_no_forced_default(self):
        ...
        assert "default" not in asmb8_redirection.argument_spec()["kvm_secure"]

    def test_send_get_web_token_defaults_true(self):
        assert asmb8_redirection.argument_spec()["send_get_web_token"]["default"] is True
...
class TestDecodedFrameAlwaysFailsHonestly:
    ...
    def test_decoded_frame_never_calls_resolve_token_and_security(self, monkeypatch):
        ...
        monkeypatch.setattr(asmb8_redirection, "resolve_token_and_security", spy)
...
class TestCheckMode:
    def test_handshake_only_check_mode_touches_no_network(self, monkeypatch):
        ...
        monkeypatch.setattr(asmb8_redirection, "resolve_token_and_security", spy)
        monkeypatch.setattr(asmb8_redirection, "build_kvm_transport", spy)
...
class TestHandshakeOnlyCapture:
    def test_returns_channel_facts_and_never_writes_a_frame(self, monkeypatch):
        ...
        assert result["operation"]["action"] == "asmb8_redirection.capture"
    ...
    def test_client_username_default_is_passed_to_open_channel(self, monkeypatch):
        ...
        monkeypatch.setattr(asmb8_redirection, "default_client_username", lambda: "detected-os-user")
        _run_ok(dict(BASE_ARGS))
...
class TestResolveTokenAndSecurity:
    def test_token_supplied_without_kvm_secure_raises_protocol_error(self, monkeypatch):
        monkeypatch.setattr(asmb8_redirection, "resolve_local_ip", lambda host: "10.1.1.1")
        with pytest.raises(ProtocolError):
            asmb8_redirection.resolve_token_and_security({"host": "10.0.0.5", "token": TOKEN, "kvm_secure": None})

    def test_token_supplied_with_kvm_secure_is_used_as_is(self, monkeypatch):
        monkeypatch.setattr(asmb8_redirection, "resolve_local_ip", lambda host: "10.1.1.1")
        token, kvm_secure, client_ip = asmb8_redirection.resolve_token_and_security({"host": "10.0.0.5", "token": TOKEN, "kvm_secure": True})
        assert token == TOKEN
    ...
    def test_no_token_mints_one_via_login_and_allocate_media_session(self, monkeypatch):
        ...
        monkeypatch.setattr(asmb8_redirection, "build_asp_client", lambda params: fake_asp)

        params = dict(BASE_ARGS, kvm_secure=None, token=None)
        token, kvm_secure, client_ip = asmb8_redirection.resolve_token_and_security(params)
    ...
    def test_explicit_kvm_secure_overrides_what_the_jnlp_reported(self, monkeypatch):
        ...
        _token, kvm_secure, _client_ip = asmb8_redirection.resolve_token_and_security(params)
    ...
    def test_missing_token_in_jnlp_response_is_a_classified_protocol_error(self, monkeypatch):
        ...
        params = dict(BASE_ARGS, kvm_secure=None, token=None)
        with pytest.raises(ProtocolError):
            asmb8_redirection.resolve_token_and_security(params)
